#!/usr/bin/env bash
# Resumable client for production FOTHEIDIL over the complete IWSLT dev set.
set -u
...
health=$(curl -sS --max-time 10 http://127.0.0.1:8000/health)
printf 'Production FOTHEIDIL health: %s\n' "${health}"
